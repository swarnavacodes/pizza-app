...
description: Use when authoring, reviewing, or applying Terraform HCL for AWS resources via Floci. Triggers on "terraform", "tf", "infra", "hcl", "Floci endpoint".
---

# Terraform + Floci Skill
...
- Planning/applying against Floci (`http://localhost:4566`)

## Hard rules
1. **Terraform only** — no `awslocal`/`aws` CLI for resource creation; CLI is fine for read inspection.
2. Every provider block must set Floci endpoints for all services used.
3. `access_key = "test"`, `secret_key = "test"`, `skip_*` flags required.
4. `s3_use_path_style = true` (Floci S3 needs path-style).
...
## Floci gotchas
- EventBridge pattern matching + archives: Floci **Pro**. Community: use SNS→SQS fanout.
- Step Functions: Floci **Pro**. Community: SQS chains + Lambda orchestration.
