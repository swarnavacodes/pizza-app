...
A distributed serverless pizza management system built on AWS (simulated via **Floci**) using **Terraform** as the sole Infrastructure-as-Code tool. Demonstrates end-to-end agentic development covering order placement, kitchen operations, delivery management, partner coordination, reporting, and promotional offers.
...
| IaC              | **Terraform** (HCL) targeting Floci          |
| Local AWS        | Floci (Pro features documented; community fallbacks noted) |
...
├── providers.tf            # aws provider with endpoints指向 Floci
...
### 3.1 Flocivider Config (providers.tf)
```hcl
provider "aws" {
  region                      = var.region
  access_key                  = "test"
...
.github/workflows/infra-validate.yml
  ...
  → tflint
  → tfsec
  → terraform plan (against Floci container in CI)
```
...
- In Floci dev: stored in `.env.local` (gitignored)
...
|------------------------|-----------------------------------------|------------------------------------------------------------|
| `Floci-skill`     | "Floci", "local aws"               | Spin up Floci, configure endpoints, validate mocks   |
...
| Agent           | Role                                                                 |
|-----------------|----------------------------------------------------------------------|
| `infrastructure`| Author & apply Terraform against Floci; emit outputs          |
...
| `devops`        | Docker compose for Floci, CI workflows, awslocal helpers      |
...
- **Costs**: Floci only for dev; prod AWS apply requires manual gate
...
**Goal**: Floci running, Terraform foundation modules, basic order flow.

1. Project scaffold: monorepo (pnpm workspaces), `infrastructure/`, `packages/`
2. Docker Compose with Floci service + init container
3. Terraform: `versions.tf`, `providers.tf` (Floci endpoints), foundation modules (iam, dynamodb, s3, sqs, sns, eventbridge)
...
3. Partner integration: outbound order sync (mock partner API in Floci), inbound webhook queue
...
6. Hardening: load tests (artillery against Floci), chaos on SQS DLQ, full Playwright E2E suite
...
├── docker/
│   └── docker-compose.yml        # Floci + init container
...
**Learning outcomes**: master Terraform module design, event-driven serverless, DynamoDB access patterns, Floci workflows, agentic dev orchestration.
...
6. Scaffold monorepo + Docker Compose for Floci.
7. Begin Phase 1.
...
| Area | Status |
|------|--------|
| opencode config (7 agents, 7 skills, guardrails, playwright MCP) | Done |
| Monorepo scaffold (pnpm workspaces, TS strict base) | Done |
| Floci compose + seed scripts + host seed.ps1 | Done (verified on running Floci 1.6.0) |
| Terraform foundation: 9 DynamoDB tables, 5 SQS+DLQ, 2 SNS, EventBridge bus, 2 S3 | Applied on Floci |
| Order service: create/get lambdas, transactional idempotency, OrderPlaced event | Done, smoke-tested via API Gateway |
| Kitchen / delivery / offer / payment / reporting / partner services | Phase 2+ |

### Dev environment facts (Phase 1)
- Floci endpoint: `http://localhost:4566` (community, always-free; LocalStack Community was EOL'd Mar 2026 � Floci is the drop-in).
- API Gateway v2 (HTTP) with `payload_format_version = "1.0"` (verified working on Floci; 2.0 produced escaped-quote bodies).
- X-Ray not available on Floci: tracer is opt-in via `XRAY_ENABLED=true` env (default off); observability via CloudWatch Logs + correlation IDs.
- DynamoDB transactions verified working on Floci (used for idempotent order creation).
- Lambda containers reach the emulator at `http://floci:4566` (Floci docker network), injected via `AWS_ENDPOINT_URL`.
- Order API: `POST /orders`, `GET /orders/{orderId}` under `http://localhost:4566/execute-api/<api-id>/$default`.
